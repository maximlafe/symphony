...
symphony-handoff-check:
	...
	cd $(ELIXIR_DIR) && $(MISE) exec -- mix handoff.check --issue "$$ISSUE_ID" --workpad "$$WORKPAD_FILE" --repo "$$REPO" --pr "$$PR_NUMBER" $(if $(MANIFEST_FILE),--manifest "$(MANIFEST_FILE)",) $(if $(EXECUTION_EVIDENCE_RUN_TOKEN),--execution-evidence-run-token "$(EXECUTION_EVIDENCE_RUN_TOKEN)",)
