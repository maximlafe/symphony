defmodule SymphonyElixir.WorkspaceAndConfigTest do
  ...
  test "issue cleanup can preserve the exact workspace while removing namespaced tmp artifacts" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-retained-issue-artifact-cleanup-#{System.unique_integer([:positive])}"
      )

    issue_identifier = "LET-267"
    workspace = Path.join(workspace_root, issue_identifier)
    issue_tmp_dir = Path.join("/tmp", "symphony-#{issue_identifier}-#{System.unique_integer([:positive])}")
    ...
    try do
      ...
      assert :ok = Workspace.cleanup_issue_artifacts(issue_identifier, remove_workspace: false)
  ...
  test "linear client applies assignee id filter directly in team-scope query" do
    assignee_filter = %{
    ...
    graphql_fun = fn query, variables ->
      send(self(), {:linear_team_scope_query_with_assignee_id, query, variables})
  ...
  test "linear client applies assignee email filter directly in team-scope query" do
    assignee_filter = %{
      configured_assignee: "dev@example.com",
      match_values: MapSet.new(["dev@example.com"]),
      query_filter: {:email, "dev@example.com"}
    }

    graphql_fun = fn query, variables ->
      send(self(), {:linear_team_scope_query_with_assignee_email, query, variables})
    ...
    assert_receive {:linear_team_scope_query_with_assignee_email, query, ^expected_variables}
    assert query =~ "SymphonyLinearTeamPollByAssigneeEmail"
    assert query =~ "assignee: {email: {eq: $assigneeEmail}}"
  end
