defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server acknowledges symphony/request_lead and emits a lead_requested event" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-request-lead-#{System.unique_integer([:positive])}"
      )

    try do
      workspace_root = Path.join(test_root, "workspaces")
      workspace = Path.join(workspace_root, "MT-LEAD")
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-request-lead.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      test_pid = self()
      ...
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-request-lead.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
        ...
        case \"$count\" in
          ...
          4)
            ...
            printf '%s\\n' '{\"id\":221,\"method\":\"symphony/request_lead\",\"params\":{\"reason\":\"need help with blockers\"}}'
            ;;
          5)
      ...
      issue = %Issue{
        id: "issue-request-lead",
        identifier: "MT-LEAD",
        title: "Lead request",
        description: "Ensure worker help requests are acknowledged",
        state: "In Progress",
        url: "https://example.org/issues/MT-LEAD",
        labels: ["backend"]
      }

      assert {:ok, _result} =
               AppServer.run(workspace, "Request lead help", issue,
      ...
      assert_received {:codex_message,
                       %{
                         ...
                         reason: "need help with blockers",
                         payload: %{"method" => "symphony/request_lead"}
                       }}
      ...
      assert Enum.any?(lines, fn line ->
               if String.starts_with?(line, "JSON:") do
                 ...
                 payload["id"] == 221 and get_in(payload, ["result", "acknowledged"]) == true
               else
                 false
               end
             end)
