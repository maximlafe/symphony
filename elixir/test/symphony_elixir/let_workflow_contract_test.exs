defmodule SymphonyElixir.LetWorkflowContractTest do
  ...
  test "LET workflow routes todo by mode labels and keeps spec prep optional" do
    ...
    assert prompt =~ "`artifact_revision` must equal `plan_revision`"
    ...
    assert prompt =~ "issue description is task-spec-only"
    assert prompt =~ "description lists must be plain bullets only (`- ...`); do not use markdown checkboxes (`- [ ]`, `- [x]`) in the issue description"
    assert prompt =~ "do not write managed markers, workpad-style progress notes, `Execution Evidence`, `Checkpoint`, or `## Рабочий журнал Codex` sections into the description"
    assert prompt =~ "for `mode:plan` and legacy spec-prep path, `Acceptance Matrix`,"
    assert prompt =~ "`Proof Mapping`, and `Остаточные риски` are mandatory"
    assert prompt =~ "each `Acceptance Matrix` data row must explicitly include"
    assert prompt =~ "`required_before` with one of `review` or `done`"
    assert prompt =~ "proof_type` aligned with `Proof Mapping`"
    assert prompt =~ "| id | scenario | expected_outcome | proof_type | proof_target | proof_semantic | required_before |"
    assert prompt =~ "for `mode:plan` and legacy spec-prep path with gate `planning.swarm_assist_enabled=true`"
    assert prompt =~ "artifact_revision` must equal `plan_revision`"
    assert prompt =~ "keep `## Symphony` as the last H2 section"
    assert prompt =~ "trailing non-heading uploads/media after this section are allowed and must remain untouched"
  ...
  test "research, plan, and execute mode skills are thin entrypoints and use canonical contract" do
    ...
    assert plan_skill =~ "every `Acceptance Matrix` data row explicitly sets"
    assert plan_skill =~ "`required_before` to `review` or `done`"
    assert plan_skill =~ "mapping type matches `Acceptance Matrix.proof_type`"
