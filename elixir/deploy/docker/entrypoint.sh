...
export SYMPHONY_SOURCE_REPO_URL="${SYMPHONY_SOURCE_REPO_URL:-$SOURCE_REPO_URL}"

if [ "${MIX_ENV:-dev}" = "prod" ]; then
  : "${SYMPHONY_SECRET_KEY_BASE:?SYMPHONY_SECRET_KEY_BASE is required when MIX_ENV=prod}"
  : "${SYMPHONY_PUBLIC_HOST:?SYMPHONY_PUBLIC_HOST is required when MIX_ENV=prod}"
fi
