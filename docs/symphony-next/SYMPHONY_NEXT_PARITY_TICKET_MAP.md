...
| `PARITY-14` | `in_progress` | `parity/parity-14-actionable-feedback-classification` | `-` | `-` | Explicit review-feedback classification (`review_state_summary`, `actionable_feedback_state`, item-level `classification`) wired to workflow guards (`ControllerFinalizer`, `HandoffCheck`) with backward-compatible bool fallback; deterministic/live fixtures + dedicated parity suite + full validation (`make all`) are green locally; pending PR/CI/merge. Evidence: `docs/symphony-next/evidence/PARITY-14/PARITY-14_EVIDENCE_2026-04-26.md` |
