...
- Terminal issue cleanup removes only the exact issue workspace plus allowlisted issue-prefixed
  task artifacts (default: `/tmp/symphony-<ISSUE>-*`, `/var/tmp/symphony-<ISSUE>-*`)
...
- Terminal state stops running agent and cleans only issue-scoped task artifacts after the issue no
  longer has running or retrying session state
